    ...
    namespace
      {
        class BinaryOpExpr final
         ...
         public:
          ...
          double Evaluate(const CellLookup &cell_lookup) const override {
            switch (type_) {
              case Add:return lhs_->Evaluate(cell_lookup) + rhs_->Evaluate(cell_lookup);
              case Subtract:return lhs_->Evaluate(cell_lookup) - rhs_->Evaluate(cell_lookup);
              case Multiply:return lhs_->Evaluate(cell_lookup) * rhs_->Evaluate(cell_lookup);
              case Divide:
        ...
        class UnaryOpExpr final
         ...
         public:
          ...
          double Evaluate(const CellLookup &cell_lookup) const override {
            switch (type_) {
              case UnaryMinus:return -operand_->Evaluate(cell_lookup);
              case UnaryPlus:return operand_->Evaluate(cell_lookup);
              default:break;
            }
        ...
        class ParseASTListener final
            : public FormulaBaseListener {
         public:
...
[[maybe_unused]] void FormulaAST::PrintCells(std::ostream &out) const {
  for (auto cell: cells_) {
    out << cell.ToString() << ' ';
  }
}

[[maybe_unused]] void FormulaAST::Print(std::ostream &out) const {
